...
class CNN:
    ...
    def __init__(self):
        self.layer1 = Convolutional(5, 4, 28)
        self.layer2 = MaxPool(4, 2)
        self.layer3 = Convolutional(3, 4, 12)
        self.layer4 = MaxPool(4, 2)
        self.layer5 = Convolutional(2, 4, 5)
        self.layer6 = Lineal(4*4*4, 32)
        self.layer7 = Lineal(32, 10)
    ...
    def reLU(self, x):
        reLU_mask = (x > 0).astype(np.uint8)
        return x * reLU_mask, reLU_mask

    def softmax(self, x):
        exps = np.exp(x - np.max(x))
        self.softmax_output = exps/np.sum(exps)
